...
if (isset($_SESSION['login'])) {
    ...
    $user = getUser($_SESSION['login'], $conn);

    # Salvando user_id em sessão para usar em outras partes
    $_SESSION['codigo'] = $user['codigo'];

    # Buscando conversas do usuário logado
    $conversations = getConversation($user['codigo'], $conn);
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
    ...
    <div>
      <div class="d-flex mb-3 p-3 bg-light justify-content-between align-items-center">
        <div class="d-flex align-items-center">
          <!-- Sem imagem de perfil no banco -->
          <h3 class="fs-xs m-2"><?= $user['nomeUsuario'] ?></h3> 
        </div>
        ...
        <?php if (!empty($conversations)) { ?>
          <?php foreach ($conversations as $conversation){ ?>
            <li class="list-group-item">
              <a href="chat.php?user=<?= $conversation['nomeUsuario'] ?>"
                 class="d-flex justify-content-between align-items-center p-2">
                <div class="d-flex align-items-center">
                  <!-- Sem imagem de perfil no banco -->
                  <h3 class="fs-xs m-2">
                    <?= $conversation['nomeUsuario'] ?><br>
